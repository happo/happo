...
export interface TakeDOMSnapshotOptions {
  ...
  strategy?: 'hoist' | 'clip';
  /**
   * Extends the default pseudo-state handling by automatically detecting and
   * applying data attributes for elements currently in `:hover`, `:active`,
   * and `:focus-visible` pseudo states, and by doing deeper focus traversal
   * (e.g. into shadow DOM) when determining which element should receive
   * `data-happo-focus`.
   *
   * Note: basic focus handling (`data-happo-focus` based on `activeElement`)
   * is always applied regardless of this option. Leaving this option on means
